...
static void _get_twin_async(DEVICE_DESIRED_DATA* device_desired_data)
{
    ...
    if (iothub_moduleclient_handle)
    {
        result = IoTHubModuleClient_GetTwinAsync(iothub_moduleclient_handle, _device_twin_callback, device_desired_data);
    }
    else
    {
        result = IoTHubDeviceClient_GetTwinAsync(iothub_deviceclient_handle, _device_twin_callback, device_desired_data);
    }
...
void _client_create_with_properies_and_send_d2c(IOTHUB_PROVISIONED_DEVICE* device_to_use, MAP_HANDLE mapHandle)
{
